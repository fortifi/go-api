...
func (m *OrderProductsPayload) validateModifySubscriptions(formats strfmt.Registry) error {
	if typeutils.IsZero(m.ModifySubscriptions) { // not required
		return nil
	}

	for i := 0; i < len(m.ModifySubscriptions); i++ {
		if typeutils.IsZero(m.ModifySubscriptions[i]) { // not required
			continue
		}
...
func (m *OrderProductsPayload) validateNamesProductPriceFids(formats strfmt.Registry) error {
	if typeutils.IsZero(m.NamesProductPriceFids) { // not required
		return nil
	}

	for i := 0; i < len(m.NamesProductPriceFids); i++ {
		if typeutils.IsZero(m.NamesProductPriceFids[i]) { // not required
			continue
		}
...
func (m *OrderProductsPayload) validateQuantityProductPriceFids(formats strfmt.Registry) error {
	if typeutils.IsZero(m.QuantityProductPriceFids) { // not required
		return nil
	}

	for i := 0; i < len(m.QuantityProductPriceFids); i++ {
		if typeutils.IsZero(m.QuantityProductPriceFids[i]) { // not required
			continue
		}
