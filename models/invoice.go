...
func (m *Invoice) validateCreditNotes(formats strfmt.Registry) error {

	if typeutils.IsZero(m.CreditNotes) { // not required
		return nil
	}

	for i := 0; i < len(m.CreditNotes); i++ {
		if typeutils.IsZero(m.CreditNotes[i]) { // not required
			continue
		}
...
func (m *Invoice) validateInvoiceDate(formats strfmt.Registry) error {
	...
	if typeutils.IsZero(m.InvoiceDate) { // not required
		return nil
	}
...
func (m *Invoice) validateInvoiceItems(formats strfmt.Registry) error {
	...
	if typeutils.IsZero(m.InvoiceItems) { // not required
		return nil
	}

	for i := 0; i < len(m.InvoiceItems); i++ {
		if typeutils.IsZero(m.InvoiceItems[i]) { // not required
			continue
		}
...
func (m *Invoice) validatePaymentDate(formats strfmt.Registry) error {
	...
	if typeutils.IsZero(m.PaymentDate) { // not required
		return nil
	}
...
func (m *Invoice) contextValidateCreditNotes(ctx context.Context, formats strfmt.Registry) error {
