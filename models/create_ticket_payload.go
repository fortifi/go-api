...
func (m *CreateTicketPayload) validateBcc(formats strfmt.Registry) error {
	if typeutils.IsZero(m.Bcc) { // not required
		return nil
	}

	for i := 0; i < len(m.Bcc); i++ {
		if typeutils.IsZero(m.Bcc[i]) { // not required
			continue
		}
...
func (m *CreateTicketPayload) validateCc(formats strfmt.Registry) error {
	if typeutils.IsZero(m.Cc) { // not required
		return nil
	}

	for i := 0; i < len(m.Cc); i++ {
		if typeutils.IsZero(m.Cc[i]) { // not required
			continue
		}
