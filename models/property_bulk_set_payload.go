...
func (m *PropertyBulkSetPayload) validateDecrementCounters(formats strfmt.Registry) error {
	if typeutils.IsZero(m.DecrementCounters) { // not required
		return nil
	}

	for i := 0; i < len(m.DecrementCounters); i++ {
		if typeutils.IsZero(m.DecrementCounters[i]) { // not required
			continue
		}
...
func (m *PropertyBulkSetPayload) validateIncrementCounters(formats strfmt.Registry) error {
	if typeutils.IsZero(m.IncrementCounters) { // not required
		return nil
	}

	for i := 0; i < len(m.IncrementCounters); i++ {
		if typeutils.IsZero(m.IncrementCounters[i]) { // not required
			continue
		}
