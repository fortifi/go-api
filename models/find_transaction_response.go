...
func (m *FindTransactionResponse) validateDiscounts(formats strfmt.Registry) error {
	if typeutils.IsZero(m.Discounts) { // not required
		return nil
	}

	for i := 0; i < len(m.Discounts); i++ {
		if typeutils.IsZero(m.Discounts[i]) { // not required
			continue
		}
...
func (m *FindTransactionResponse) validateSubscriptions(formats strfmt.Registry) error {
	if typeutils.IsZero(m.Subscriptions) { // not required
		return nil
	}

	for i := 0; i < len(m.Subscriptions); i++ {
		if typeutils.IsZero(m.Subscriptions[i]) { // not required
			continue
		}
...
func (m *FindTransactionResponse) validateTaxes(formats strfmt.Registry) error {
	if typeutils.IsZero(m.Taxes) { // not required
		return nil
	}

	for i := 0; i < len(m.Taxes); i++ {
		if typeutils.IsZero(m.Taxes[i]) { // not required
			continue
		}
